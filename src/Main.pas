...
uses
  ...
  AboutDelphiArea, AlignDlg, SizeDlg, PropDlg;
...
begin
  { 2020 }
  if not Assigned(PropDialog) then begin
    PropDialog := TPropDialog.Create(nil);
    PropDialog.Show;
    PropDialog.DesignerExecute(SimpleGraph);  { Designer Viewport }
  end;
...
begin
  simplegraph.DefaultLinkClass := TGraphLink;
...
procedure TMainForm.EditMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if ([ssLeft] = Shift) then
  begin
      TEdit(sender).Left := TEdit(sender).Left + X - WndPos.x;
      TEdit(sender).Top := TEdit(sender).Top + Y - WndPos.y;
  end
end;
...
var
  EditArea : TEditArea;
begin
  EditArea := TEditArea.Create(self);
  EditArea.Parent := SimpleGraph;
  EditArea.Show;
  // MouseDown, MouseMove
  //SimpleGraph.ActiveEditArea := EditArea;
  SimpleGraph.AddEditArea(EditArea);
end;

procedure TMainForm.FormShow(Sender: TObject);
begin
  if PropDialog <> nil then
  begin
    fdockmanager := TDockManager.create(self);
    fdockmanager.DockDlg := PropDialog;  
    PropDialog.Show;
        //KeSourceTreeDialog.SourceTabSheetShow(self);
  end;
end;
...
var
  Points: Tobjectlist;
  CatmullRom:  TCompositeCurve;
begin
  Points := TObjectList.Create();
  //Selected := nil;
  Randomize();
  strtoint('1');
  CatmullRom := TCompositeCurve.Create(simplegraph,points,SimpleGraph.Canvas, TCatmullRomCurve, 3);
  //CatmullRom.ControlPoints[0] := TControlPoint.Create(SimpleGraph.Canvas, 400+Random(100),  20+Random(50), 5);
  //CatmullRom.ControlPoints[1] := TControlPoint.Create(SimpleGraph.Canvas, 400+Random(100), 100+Random(50), 5);
  //CatmullRom.ControlPoints[2] := TControlPoint.Create(SimpleGraph.Canvas, 400+Random(100), 160+Random(50), 5);
  //CatmullRom.ControlPoints[3] := TControlPoint.Create(SimpleGraph.Canvas, 400+Random(100), 250+Random(50), 5);
  //CatmullRom.ControlPoints[4] := TControlPoint.Create(SimpleGraph.Canvas, 400+Random(100), 320+Random(50), 5);
  //CatmullRom.ControlPoints[5] := TControlPoint.Create(SimpleGraph.Canvas, 400+Random(100), 400+Random(50), 5);


end;

procedure TMainForm.ToolButton50Click(Sender: TObject);
begin
...
begin
  simplegraph.DefaultLinkClass := TSplineLink;
  simplegraph.CommandMode := cmInsertLink;
end;

procedure TMainForm.ToolButton51Click(Sender: TObject);
begin
  simplegraph.DefaultLinkClass := TBSplineLink;
  simplegraph.CommandMode := cmInsertLink;
end;
